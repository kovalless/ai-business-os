...
export default function PeoplePage() {
  ...
  const attention = useMemo(() => people.filter((p) => p.attention), []);
  const drifting = useMemo(
    () => people.filter((p) => p.standing === "drifting" || p.lastContactDays > 60),
    [],
  );
  ...
  const list = useMemo(() => {
    ...
    return [...filtered].sort((a, b) => {
      ...
      if (sort === "quiet") return b.lastContactDays - a.lastContactDays;
      return a.lastContactDays - b.lastContactDays;
    });
  }, [view, q, sort, attention, drifting]);
    ...
    <Room
      ...
      margin={
        <div className="flex flex-col gap-bay">
          <MarginNote
            text="Brouwer & Zn ordered every four to six weeks for four years, then stopped in March. No complaint was ever logged."
            confidence="known"
            actions={[
              { label: "Draft a note to Ilse", href: "/table" },
              { label: "Show me why", href: "/people/brouwer" },
            ]}
          />
          <Proposal confidence="estimated" method="Two messages have gone unanswered for more than 5 days.">
            Wies Ansems asked on 22 July whether she may use your workshop photographs. Nobody has
            replied.
          </Proposal>
        </div>
      }
            ...
            <Row key={p.id} href={`/people/${p.id}`}>
              <div className="flex items-start gap-rise">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-tick text-caption text-ink-2">
                    {p.attention ?? p.relationship}
                  </p>
                </div>
                <div className="hidden shrink-0 text-right sm:block">
                  <span
                    className={cx(
                      "num block text-bodysm",
                      p.outstanding ? "text-ink" : "text-ink-4",
                    )}
                  >
                    {p.outstanding ? money(p.outstanding) : "\u2014"}
                  </span>
                  <span className="block text-caption text-ink-3">
                    {relativeDays(p.lastContactDays)}
                  </span>
                </div>
              </div>
            </Row>
