...
import { docs, notes } from "@/lib/data";
...
export default function RecordPage() {
  ...
  const list = useMemo(() => {
    ...
    if (!term) return base;
    return docs.filter(
      (d) => d.title.toLowerCase().includes(term) || d.body.toLowerCase().includes(term),
    );
  }, [q, view]);
    ...
    <Room
      ...
      margin={
        <div className="flex flex-col gap-bay">
          <Proposal confidence="estimated" method="Written from two years of Dijkman order history.">
            Dijkman raises in July and January, usually 4 to 6%, with no notice. Worth filing before
            next January.
          </Proposal>
          <div>
    ...
    >
      ...
      {q ? (
        <p className="mt-step text-caption text-ink-3">
          Searching every entry, not just this tab.
        </p>
      ) : null}
