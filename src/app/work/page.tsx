...
import { priorityOf, tasks as seed, upcoming, workCalendar } from "@/lib/data";
...
export default function WorkPage() {
  ...
  const groups = (["now", "soon", "whenever"] as const).map((p) => ({
    p,
    list: open.filter((t) => (priorityOf[t.id] ?? "soon") === p),
  }));
    ...
    <Room
        ...
        <div className="flex items-center gap-rise">
          ...
          <Shelf
            ...
            items={[
              ...
              { key: "Joris", label: "Joris" },
              { key: "Ruben", label: "Ruben" },
              { key: "Marit", label: "Marit" },
            ]}
          />
        </div>
      }
      margin={
        <div className="flex flex-col gap-bay">
          <Proposal confidence="estimated" method="Steel rose 6% on 12 July; three quotes predate it.">
            The three requotes are the only tasks with money attached today. Together they recover
            about &euro;2,840 of margin.
          </Proposal>
          <div>
